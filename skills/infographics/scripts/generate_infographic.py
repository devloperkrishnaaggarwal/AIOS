#!/usr/bin/env python3
"""
Generate professional infographics using Kie AI Nano Banana 2.

This script generates infographics with smart iterative refinement:
- Uses Nano Banana 2 via Kie AI async task API for generation
- Optionally uses OpenRouter for quality review
...
def list_options():
    ...
    print("""
+==============================================================================+
|              INFOGRAPHIC GENERATION OPTIONS (Nano Banana 2)                   |
+==============================================================================+
...
------------------------------------------------------------------------------
  1K            Standard resolution (fastest)
  2K            High resolution (default, recommended)
  4K            Ultra-high resolution (slowest, best quality)

------------------------------------------------------------------------------
Examples:
  python generate_infographic.py "5 benefits of exercise" -o benefits.png --type list
  python generate_infographic.py "AI adoption 2020-2025" -o timeline.png --type timeline --style technology
  python generate_infographic.py "Product comparison" -o compare.png --type comparison --resolution 4K

Environment Variables:
  KIE_API_KEY           Required - Kie AI API key (https://kie.ai/api-key)
  OPENROUTER_API_KEY    Optional - For quality review and research features
...
How it works:
  ...
  3. Nano Banana 2 generates it via Kie AI async API:
     - Creates a task, polls for completion
     - Downloads the result image
     - Optional quality review via OpenRouter
     - Smart iteration (only regenerates if below threshold)
...
Examples:
  ...
  python generate_infographic.py "Global AI market size" -o ai_market.png --type statistical --research

  # 4K ultra-high resolution
  python generate_infographic.py "Tech trends 2026" -o trends.png --type list --resolution 4K

  # Use Google search grounding
  python generate_infographic.py "Latest AI news" -o ai_news.png --google-search
...
Environment Variables:
  ...
  OPENROUTER_API_KEY    Optional - For quality review
        """
    )
    ...
    parser.add_argument("--resolution", default="2K", choices=RESOLUTIONS,
                       help="Output resolution (default: 2K)")
    parser.add_argument("--format", default="png", choices=OUTPUT_FORMATS,
    ...
    parser.add_argument("--image-input", action="append",
                       help="Reference image URL(s) (can specify multiple)")
    parser.add_argument("--iterations", type=int, default=3,
                       help="Maximum refinement iterations (default: 3)")
    parser.add_argument("--api-key",
                       help="Kie AI API key (or use KIE_API_KEY env var)")
    parser.add_argument("--openrouter-key",
                       help="OpenRouter API key for review (or use OPENROUTER_API_KEY env var)")
    parser.add_argument("-v", "--verbose", action="store_true",
                       help="Verbose output")
    parser.add_argument("--research", "-r", action="store_true",
                       help="Research the topic first (requires OPENROUTER_API_KEY)")
    ...
    if args.google_search:
        cmd.append("--google-search")
    if args.image_input:
        for img_url in args.image_input:
            cmd.extend(["--image-input", img_url])
    if args.iterations != 3:
        cmd.extend(["--iterations", str(args.iterations)])
    if api_key:
        cmd.extend(["--api-key", api_key])
    if args.openrouter_key:
        cmd.extend(["--openrouter-key", args.openrouter_key])
    if args.verbose:
        cmd.append("-v")
    if args.research:
        cmd.append("--research")
